...
async function seed(o: Partial<TalentProfilePublic> = {}, over: { expiresAt?: string } = {}) {
  const p = profile(o);
...
describe("search", () => {
  ...
  it("filters by category, state, city and availability", async () => {
    expect((await talent.search({ category: "belleza" })).total).toBe(2);
    expect((await talent.search({ state: "TX" })).total).toBe(2);
    expect((await talent.search({ city: "Miami" })).total).toBe(1);
    expect((await talent.search({ availability: "un_mes" })).total).toBe(1);
  });

  it("combines filters rather than widening them", async () => {
    expect((await talent.search({ category: "belleza", state: "TX" })).total).toBe(1);
  });
